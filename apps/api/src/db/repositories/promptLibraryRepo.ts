...
const promptLibrarySchemaStatements = [
...
export async function ensurePromptLibrarySchema(db: D1Database) {
  if (schemaReadyDbs.has(db as object)) {
    return;
  }

  for (const statement of promptLibrarySchemaStatements) {
    await db.prepare(statement).run();
  }

  schemaReadyDbs.add(db as object);
}
