...
test("user opens Etsy prep from product detail, generates the prompt pack, and saves the workspace", async ({ page }) => {
  ...
  await expect(page.getByText(/rulebook: etsy-prompt-pack-v9/i)).toBeVisible();
  ...
  await expect(page.getByText(/do not mention color or capacity in tags/i)).toBeVisible();
